...
def build_task(data: dict) -> Task:
    ...
    if status == TaskStatus.ASSIGNED_TO_PROVIDER:
        status_payload = AssignedToProviderPayload(task_status=status, provider_id=data.get('provider_id', 0))
    elif status == TaskStatus.SENT_FAILED:
        status_payload = SentFailedPayload(attempt_num=data.get('num_failed_attempts', 0))
    else:
...
class StorageManager:
    ...
    def get_task_data(self, task_id: int) -> dict:
        if self._use_supabase:
            data, _ = supabase.table(self._table_name).select('priority', 'status', 'provider_id', 'num_failed_attempts', 'info', 'log', 'result').eq('id', task_id).execute()
    ...
    def get_task_data_with_verification(self, user_id: int, task_id: str) -> dict:
        if self._use_supabase:
            data, _ = supabase.table(self._table_name).select('priority', 'status', 'provider_id', 'num_failed_attempts', 'info', 'log', 'result').eq('id', task_id).eq('user_id', user_id).execute()
    ...
    def get_tasks(self, user_id: int) -> dict:
        if self._use_supabase:
            data, _ = supabase.table(self._table_name).select('priority', 'status', 'provider_id', 'num_failed_attempts', 'info', 'log', 'result').eq('user_id', user_id).execute()
